...
1. First, you need to configure a paswordless access to your Jetson machine. The easiest way to do it is [configure SSH key-based authentication](https://www.digitalocean.com/community/tutorials/how-to-configure-ssh-key-based-authentication-on-a-linux-server).
...
Now you should be able to ssh to your host without a passowrd:
...
Finally, verify the `remote` context is accesible:
...
Open AI Workbench and connect to your local location. You should see `My Projects | local` screen where you may choose to create a new one or clone an existing. Choose `Clone Project` and put the project GitHub link (if you are planning to change the code, then I suggest forking the project first):
...
> Note: Building the AI Workbench project will take a while (about 10-15 mins on my machine). During this time the tool will download the base image, install all the project dependencies and prepare the new `project` image.

In the meantime, we can work on the Jetson images.

> Note: In theory, this part could have been automated and included into the workbench project, similarly, how the [workbench-example-hybrid-rag](https://github.com/NVIDIA/workbench-example-hybrid-rag) application does that. In practice though, this sort of orchestration is really a limitation of my chosen deployment environment (Ubuntu dev machine + Jetson machine) and would not be needed at all if I had a proper development system on a single laptop. So I decided to skip this part and focus on what is really interesting - using the models.
...
This will take you to the `llm-server` prompt that has Ollama server running. You can choose any model supported by Ollama server to be your LLM for the **script generation** task. Just keep in mind that you'll need to adjust the settings for the workbench app accordingly (we will explore it later in this guide). For now, let's download the `phi3` model. Enter in the `llm-server` prompt the commands to download and test your model:
...
Now, that we have everything ready on the Jetson machine, let's go back to the AI Workbench and see if the build process has finished. If you see `Build Ready` message at the bottom of the screen, we should be good to go. One last thing we need to do is to modify the environment variables. Go to the `Environment` tab and scroll down to the `Variables` section:

![Environment Variables](media/workbench-env.png)

Modify the environment variables so that they point to your Jetson machine - figure out the IP address of it and edit the variables for LLM and TTS server. Here, you can also change the LLM model that you want to use for **script generation** (make sure you download it with ollama server before using it).
...
P.S. I suggest you look into the code itself, play with the various LLM models (such as `llama3` and `mistral` - believe me, this was a lot of fun!) and also voices in the TTS model. Also, please let me know how this app can be improved or feel to send your PRs!
